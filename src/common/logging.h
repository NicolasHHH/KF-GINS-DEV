...
#ifndef LOGGING_H
#define LOGGING_H
// 一般来说，这个标识符是由当前头文件的名称加上一个唯一的后缀组成，用于防止头文件的重复包含。
...
#if !DCHECK_IS_ON()
// 判断DCHECK是否开启。如果DCHECK没有开启，会定义一些只在Debug模式下有效的日志宏，如果DCHECK开启，则这些Debug日志宏直接映射到相应的日志宏。
...
public:
    static void initialization(char **argv, bool logtostderr = true, bool logtofile = false) {
        ...
        // glog初始化
        google::InitGoogleLogging(argv[0]); // 文件名
    }
