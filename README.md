...
😕 **attitude error, Phi angle model, two-sample mechanization, linear assumption, coning correction, sculling motion.**
...
- IMU frame: (FRD) the origin is the IMU center, the three axes point forward-right-down
- Navigation reference frame: (NED) the origin coincides with the IMU frame, the three axes point north-east-down

**Navigation state:**

- position: (lat, long, eph) the geodetic coordinates of the IMU position in the Earth frame (latitude-longitude-ellipsoid height)
...
| 11~13   | X-Y-Z axes accelerometer scale factors | $ppm$   |
